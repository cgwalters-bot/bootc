...
/// Extract the os_id from a sort key generated by [`primary_sort_key`] or
/// [`secondary_sort_key`].
///
/// Returns `None` if the sort key isn't in the `bootc-{os_id}-{priority}`
/// format, e.g. entries written by older bootc versions which used a bare
/// `0`/`1` sort key.
pub(crate) fn os_id_from_sort_key(sort_key: &str) -> Option<&str> {
    let (os_id, priority) = sort_key.strip_prefix("bootc-")?.rsplit_once('-')?;
    let valid_priority = [SORTKEY_PRIORITY_PRIMARY, SORTKEY_PRIORITY_SECONDARY].contains(&priority);
    (valid_priority && !os_id.is_empty()).then_some(os_id)
}
...
mod tests {
    ...
    fn test_os_id_from_sort_key() {
        for os_id in ["fedora", "rhel", "fedora-coreos", "bootc"] {
            assert_eq!(os_id_from_sort_key(&primary_sort_key(os_id)), Some(os_id));
            assert_eq!(os_id_from_sort_key(&secondary_sort_key(os_id)), Some(os_id));
        }

        let cases = [
            ("bootc-rhel-0", Some("rhel")),
            ("bootc-my-custom-os-1", Some("my-custom-os")),
            // Legacy sort keys without an os_id
            ("0", None),
            ("1", None),
            ("", None),
            ("bootc-", None),
        ...
        for (sort_key, expected) in cases {
            assert_eq!(os_id_from_sort_key(sort_key), expected, "{sort_key}");
        }
    }
