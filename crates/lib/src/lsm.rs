...
/// Without libselinux, bootc behaves as if SELinux is disabled for this process.
/// Labels computed from a target root's policy are still written.
#[cfg(not(feature = "selinux"))]
pub(crate) fn selinux_enabled() -> bool {
    false
}
